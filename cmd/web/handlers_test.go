...
func TestUserSignup(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			//Make a POST /user/signup request, using the same http.Client that we used in step 1
			//(so it automatically passes the CSRF cookie with the POST request) and including
			//the CSRF token alongside the other POST data that we want to test.
